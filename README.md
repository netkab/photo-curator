...
### Likely accidents (extension 3.1.2)
...
Nothing is preselected. Inspect the original in Google Photos and select unwanted photos within a burst. With Dry run checked, **Preview selected** rehearses the operation and leaves the burst available. Uncheck Dry run and click **Trash selected**, then confirm the named photos. The tab records and approves that exact selection, starts the operation, and shows progress and Undo directly. Unselected burst photos and nearby context are protected; at least one surviving photo is required. The existing backend approval and live-trash opt-in still apply. If approval or queueing is interrupted after a review is saved, the error names that review so you can recover in Review queue. **Keep this burst** persistently hides that burst, including overlapping suggestions on later runs. No automatic trash occurs.
...
### Temporary photos and repeated attempts (3.2.0)

**Temporary photos** reads cached previews using macOS Vision text recognition. On the first run it builds a small local helper with Apple's command-line tools (`swiftc`); it does not download a model or upload images. Categories are delivery/order updates, parking/travel details, shopping/menus, possible screenshots (filename cue), documents/receipts, and other text-heavy photos. Filter by category and review groups of up to 25 photos from the same month. These are keyword-based suggestions, not proof that information has expired. Recognized text is discarded after labeling; only category labels and a cache signature are saved. Interrupted runs keep completed labels and reuse them on retry. This reader currently requires macOS; other systems show a setup error and retain existing suggestions. Recognition languages depend on the installed Vision version. Small text and unsupported languages may be missed.

**Repeated attempts** compares owned, cached previews captured within two minutes. It uses a broader perceptual-hash match plus color and aspect checks; no CLIP installation is required. Every match is compared with an anchor to prevent a chain spanning different scenes. Groups contain at most 25 photos. Sensitivity controls how different two previews may be. A suggested keeper is based on preview sharpness, then reported dimensions; it does not assess expressions, closed eyes, or emotional value. This is a broader comparison tool than Duplicates, but can overlap with unreviewed duplicate suggestions. Photos already committed to a review are excluded.

Both pages use explicit selection, Dry run, confirmed Trash and Undo. Temporary-photo groups can have every item selected, since a disposable reference need not have a duplicate keeper; the backend verifies the selection against the stored temporary group. Repeated-attempt groups require a surviving unselected photo. Category analyses replace only their own pending suggestions; kept/reviewed photos remain excluded. Nothing is selected or trashed automatically. Large-video cleanup remains outside this tool.

### Page-wide selection (3.3.0)

All three cleanup tabs have one sticky Preview/Trash button for the photos you select on the current page, including groups added with Show more. Operations default to a compact summary; expand details for history, Stop or Resume, or use Undo last. Each group has **Select entire group**, including any suggested keeper. The confirmation explicitly counts entire groups. A page selection creates one review and one operation, with atomic validation across all included groups. Unselected photos and nearby context remain protected. Changing filters or restarting analysis clears the visible selection; hidden photos are not included.

The earlier surviving-keeper requirement no longer applies when you explicitly select a whole group in these three tabs. The exception is tied to stored group membership and the exact reviewed selection at both approval and execution. Duplicate-tab behavior remains as before.
