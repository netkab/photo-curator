...
## 3.3.0 — compact operations and page-wide trash

62 backend tests and 21 extension tests pass. Added coverage for whole-group selection reaching live-enabled batches, explicit opt-in, multi-group atomic review creation, invalid-group rollback, protected partial selections, and cross-group context explicitly selected elsewhere. The UI helper test verifies one approval/apply sequence for a page, whole-group confirmation text, and no writes when confirmation is cancelled. Synthetic browser verification with 50 prior operations showed collapsed history, whole-group checkbox selection and a single updated global Trash count. UI detector and JavaScript syntax checks pass. No real trash operation was performed. Reviewer findings about failed-analysis recovery and Undo-last ordering were corrected.
